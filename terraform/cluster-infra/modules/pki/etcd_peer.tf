resource "tls_private_key" "k8s_etcd_peer" {
  count     = var.k8s_controlplane_count
  algorithm = tls_private_key.etcd_ca.algorithm
  rsa_bits  = tls_private_key.etcd_ca.rsa_bits
}

resource "tls_cert_request" "k8s_etcd_peer" {
  count           = var.k8s_controlplane_count
  private_key_pem = tls_private_key.k8s_etcd_peer[count.index].private_key_pem
...
resource "tls_locally_signed_cert" "k8s_etcd_peer" {
  count              = var.k8s_controlplane_count
  cert_request_pem   = tls_cert_request.k8s_etcd_peer[count.index].cert_request_pem
